...
#define MAX_FFA	1000
#define MAX_D	1000
//#define najlepszyWynik 13549094
//#define eps2 300000

using namespace std;

unsigned int seedFA;
double najlepszyWynik = 13549094*25;
//double eps2 = 300000*90;
double eps2 = najlepszyWynik*0.04;
...
void init_ffa(int liczbaWatkow)
{
	...
	for (i=0;i<N;i++)
	{
		for (j=0;j<D;j++)
		{
			...
			r = (double)rand_r(&seedFA) / ((double)RAND_MAX+(double)(1));
...
//        }
		...
		if (t != 1 && abs(I[0] - najlepszyWynik) <= eps2)
        {
            //cout << "\nWszedlem do ifa\n" << t << endl;
            licznik++;
            if (licznik == 10)
	...
	cout << "Koñcowa wartość funkcji celu = " << fbest << endl;
	return nbest;
}
...
void sort_ffa()
{
	...
	for(i=0;i<N-1;i++)
	{
		for(j=i+1;j<N;j++)
		{
			if(I[i] < I[j]) // zmiana znaku nierĂłwnoĹ“ci na zgodnosc z artykulem
			{
				double z = I[i];	// exchange attractiveness
				I[i] = I[j];
				I[j] = z;
...
void move_ffa(int liczbaWatkow)
{
	...
	for(i=0;i<N;i++)
	{
		...
		for(j=0;j<N;j++)
		{
			...
			r = sqrt(r);
			if(I[i] < I[j])	// brighter and more attractive // zmiana znaku nierĂłwnoĹ“ci na zgodnosc z artykulem
			{
				double beta0 = 1.0;
				beta = (beta0-betamin)*exp(-gama*pow(r, 2.0))+betamin;
				for(k=0;k<D;k++)
...
void FA(int liczbaWatkow){
	...
	cout << "KoĂ±cowa wartoĹ“Ă¦ funkcji celu = " << fbest << endl;
}
