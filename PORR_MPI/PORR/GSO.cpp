...
#define epsilon 100//400

#define MAX_N	1000
#define MAX_D	1000

unsigned int seedGSO;
...
double najlepszyWynikGSO = 13549094*25;
double eps2GSO = najlepszyWynikGSO*0.04;
...
//	{
	        ...
	        for(int i = 0; i < n; i++) {
                for(int j = 0; j < D; ++j) {
                    //rr = rand232();
                    rr = (double)rand_r(&seedGSO) / ((double)RAND_MAX+(double)(1));
...
//			}
		...
		//if (t != 1 && abs(Lc[indexNa] - najlepszyWynikGSO) <= eps2GSO){
			//cout<<"Indeks siê nie zmieni³ od "<<licznik<<" iteracja "<<t<<endl;
			++licznik;
			if(licznik == epsilon){
				cout<<"Osiągnieto warunek stopu w iteracji "<<t<<endl;
				break;
			}
		}else{
			cout<<"Nastąpiła zmiana w iteracji "<<t<<endl;
			cout << Lc[indexNa] << endl;
			licznik = 0;
			bestNa = indexNa;
		}
...
//	}
	...
	double *podajWynik = new double[D];
	cout<<endl<<"Najlepszy swietlik"<<endl;
	for (int j=0; j<D; ++j){
		cout<<X[indexNa][j]<<" ";
		podajWynik[j]=X[indexNa][j];
	}
...
double najlepszyWynik = 13549094;
double eps2 = 300000;
